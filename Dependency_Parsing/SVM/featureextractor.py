...
#    @staticmethod
#    def find_left_right_sibling(idx, arcs):
#	left_most = 1000000
#	right_most = -1
#	left_most_sibling = ''
#	right_most_sibling = ''
...
#		for (wk, r, wl) in arcs:
#		    if wk == wi and wl != wj:
#			if (wl > wj) and (wj > right_most):
#			    right_most = wl
#			    right_most_sibling = r
#		        if (wl < wj) and (wl < left_most):
#			    left_most = wl
#			    left_most_sibling = r
#	return left_most_sibling, right_most_sibling
...
#	    left_most_sibling, right_most_sibling = FeatureExtractor.find_left_right_sibling(stack_idx0, arcs)

#	    if FeatureExtractor._check_informative(left_most_sibling):
#		result.append('STK_0_LSDEP_' + left_most_sibling)
#	    if FeatureExtractor._check_informative(right_most_sibling):
#		result.append('STK_0_RSDEP_' + right_most_sibling)
...
#            left_most_sibling, right_most_sibling = FeatureExtractor.find_left_right_sibling(buffer_idx0, arcs)
#            if FeatureExtractor._check_informative(left_most_sibling):
#                result.append('BUF_0_LSDEP_' + left_most_sibling)
#            if FeatureExtractor._check_informative(right_most_sibling):
#		result.append('BUF_0_RSDEP_' + right_most_sibling)
