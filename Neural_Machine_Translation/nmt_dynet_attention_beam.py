...
class nmt_dynet_attention:
	...
	loss = []
	    ...
	    if i == 0:
		embed = vecInput(self.word_d)
		embed.set([0] * self.word_d)
    ...
    def generate(self, src_sentence):
    	...
    	# setup the sentence
        w_s = matInput(self.gru_d, self.gru_d)
        s = tanh(w_s * h_encode[0][self.gru_d:]) 
	#initilize hidden state for decoder
	c_i = self.attend(h_encode, s)
        dec = self.dec_RNN.initial_state()
	w_o = parameter(self.output_w)
	b_o = parameter(self.output_b)
	...
	for idx in beam_idx_lst:
	    ...
	    probs_lst = []
	    for i in range(K):
		c_i = self.attend(h_encode, beam_dec_lst[i].output())
		word_idx = self.tgt_word2idx[beam_sentences_lst[i][-1]]
		embed = self.target_embeddings[word_idx]
	  	beam_dec_lst[i] = beam_dec_lst[i].add_input(concatenate([embed, c_i]))
